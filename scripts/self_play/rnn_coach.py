...
class ConvRnnCoach(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.glob_encoder = ConvGlobEncoder(
            args, num_unit_type, num_cmd_type, num_resource_bin, self.prev_inst_encoder
        )
        ...
        self.glob_feat_dim = int(
            2.5 * self.args.count_hid_dim + self.glob_encoder.glob_dim
        )
        ...
        if self.coach_rule_emb_size > 0:
            ...
            self.rule_lstm = torch.nn.LSTM(
                self.coach_rule_emb_size, self.glob_feat_dim, batch_first=True
            )
        ...
        self.value = nn.utils.weight_norm(nn.Linear(self.glob_feat_dim, 1), dim=None)
    ...
    def _get_pos_candidate_inst(self, device):
        if (
            self.pos_candidate_inst is not None
    ...
    def _get_neg_candidate_inst(self, device, exclude_idx):
        if (
            self.neg_candidate_inst is not None
        ...
        select = np.random.choice(
            inst.size(0), self.args.num_neg_inst, replace=False, p=prob
        )
