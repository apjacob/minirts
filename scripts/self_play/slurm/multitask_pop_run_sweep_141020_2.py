...
JOB_SWEEP_NAME = "multitask_pop_sweep_3"
...
# Sweep params
NUM_TRIALS = 2
lifelong_series = ["80 40 20", "3 12 13", "7 21 14 80"]
rnns = ["rnn"]
train_modes = ["both", "coach"]
lrs = [5e-6, 8e-6]
...
for trial in range(NUM_TRIALS):
    for lifelong_serie, rnn, train_mode, lr in itertools.product(lifelong_series, rnns, train_modes, lrs):
        ...
        python_command = "python -u /home/gridsan/apjacob/minirts/scripts/self_play/multitask-pop.py --coach1 rnn500 " \
                  "--coach2 rnn500 " \
                  f"--executor1 {rnn} " \
                  f"--executor2 {rnn} " \
                  f"--lr {lr} " \
